            ...
            <div class="row">
                <div class="col-md-8">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="deskripsi" name="deskripsi" rows="5">{{ old('deskripsi', $laporan->meta_data['deskripsi'] ?? '') }}</textarea>
                    </div>
                ...
                <div class="col-md-4">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="periode" name="periode">
                            <option value="Januari - Desember" {{ (old('periode', $laporan->meta_data['periode'] ?? '') == 'Januari - Desember') ? 'selected' : '' }}>Januari - Desember</option>
                            <option value="Semester 1" {{ (old('periode', $laporan->meta_data['periode'] ?? '') == 'Semester 1') ? 'selected' : '' }}>Semester 1</option>
                            <option value="Semester 2" {{ (old('periode', $laporan->meta_data['periode'] ?? '') == 'Semester 2') ? 'selected' : '' }}>Semester 2</option>
                            <option value="Triwulan 1" {{ (old('periode', $laporan->meta_data['periode'] ?? '') == 'Triwulan 1') ? 'selected' : '' }}>Triwulan 1</option>
                            <option value="Triwulan 2" {{ (old('periode', $laporan->meta_data['periode'] ?? '') == 'Triwulan 2') ? 'selected' : '' }}>Triwulan 2</option>
                            <option value="Triwulan 3" {{ (old('periode', $laporan->meta_data['periode'] ?? '') == 'Triwulan 3') ? 'selected' : '' }}>Triwulan 3</option>
                            <option value="Triwulan 4" {{ (old('periode', $laporan->meta_data['periode'] ?? '') == 'Triwulan 4') ? 'selected' : '' }}>Triwulan 4</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        @if(isset($laporan->meta_data['file_path']) && $laporan->meta_data['file_path'])
                            <br><small class="text-success">
                                <i class="fas fa-file me-1"></i>File saat ini: {{ basename($laporan->meta_data['file_path']) }}
                            </small>
                        @endif
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" id="tanggal_publikasi" name="tanggal_publikasi" value="{{ old('tanggal_publikasi', $laporan->meta_data['tanggal_publikasi'] ?? '') }}">
                    </div>

                    <div class="mb-3">
                        <label for="status" class="form-label">Status</label>
                        <select class="form-control" id="status" name="status">
                            <option value="draft" {{ (old('status', $laporan->meta_data['status'] ?? '') == 'draft') ? 'selected' : '' }}>Draft</option>
                            <option value="published" {{ (old('status', $laporan->meta_data['status'] ?? '') == 'published') ? 'selected' : '' }}>Dipublikasi</option>
                            <option value="archived" {{ (old('status', $laporan->meta_data['status'] ?? '') == 'archived') ? 'selected' : '' }}>Arsip</option>
                        </select>
                    </div>
                </div>
            </div>
